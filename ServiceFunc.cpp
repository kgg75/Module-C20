...
uint8_t get_command_code(const CommandSpace* commandSpace, string& command) {	// выполняет анализ команды и возвращает соответствующий код действия
...
uint8_t get_command(const CommandSpace* commandSpace, const string msg, const string& user_name) {	// выполняет получение команды с консоли, возвращает соответствующий код действия
	cout << user_name << msg;	// вывод обращения
...
string GetTime() {	// возвращает текущее время
	...
	stringstream ss;
	ss << put_time(localtime(&t_c), DATE_TIME_FORMAT);	// "[%Y-%m-%d %H:%M:%S].\n";
	return ss.str();
}
